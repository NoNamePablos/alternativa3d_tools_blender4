...
def create_principled_material(name):
    """Create a material with Principled BSDF node setup."""
...
def add_texture_to_material(mat, image, tex_type="diffuse"):
    """Add a texture image to a node-based material.
    tex_type can be: diffuse, glossiness, normal, opacity, specular, light, reflection
    """
    if mat is None or image is None:
        return
    ...
    offset_y = -300 * (len(existing_tex) - 1)
    tex_node.location = (-400, offset_y)
    
    if tex_type == "diffuse":
        links.new(tex_node.outputs['Color'], bsdf.inputs['Base Color'])
    elif tex_type == "normal":
        normal_map = nodes.new(type='ShaderNodeNormalMap')
        normal_map.location = (-150, offset_y)
        ...
        # Lightmap - connect to emission or just keep as reference
        pass
    elif tex_type == "reflection":
...
def checkBMesh():
	# Always True in Blender 2.63+/4.x
	return True
...
class ASExporter(bpy.types.Operator):
	...
	def execute(self, context):
		...
		try:
			...
			file.close()
			print(".as export time: %.2f" % (time.perf_counter() - time1))
...
def GetMaterialTexture(Material):
	img_files = []
	if Material:
		# Node-based material texture reading (Blender 4.x)
		if Material.use_nodes:
			for node in Material.node_tree.nodes:
				if node.type == 'TEX_IMAGE' and node.image:
					if node.image.source == "FILE" and node.image.filepath:
						img_files.append(os.path.basename(node.image.filepath))
...
def getCommonData(Config,obj,flipUV=1):
	...
	verts = mesh.vertices
	Materials = mesh.materials
	hasFaceUV = len(mesh.uv_layers) > 0
		...
		for uvlayer in mesh.uv_layers:
			...
			if uvprocess == True:
				...
				for uv_index in range(len(mesh.tessfaces)):
					...
					#face = mesh.uv_layers.active.data[uv_index]
		...
		for face in mesh.faces:
			if len(face.vertices) > 0:
				...
				for i in range(len(face.vertices)):
					...
					hasFaceUV = len(mesh.uv_layers) > 0
					if hasFaceUV:
						uv = [mesh.uv_layers.active.data[face.index].uv[i][0], mesh.uv_layers.active.data[face.index].uv[i][1]]
...
class A3DExporter(bpy.types.Operator):
	...
	def execute(self, context):
		...
		try:
			...
			file.close()
			print(".a3d export time: %.2f" % (time.perf_counter() - time1))
		except Exception as e:
			print(e)
			file.close()
		return {'FINISHED'}
		...
		for obj in objs_a3ditems:
			if obj["a3dtype"] == 'A3DSprite3D':
				...
				for x in range(len(mts)):
					...
					# Node-based material texture reading (Blender 4.x)
					if mats[x].use_nodes:
						for node in mats[x].node_tree.nodes:
							if node.type == 'TEX_IMAGE' and node.image is not None:
								name = node.label.lower() if node.label else "diffuse"
								a3dstr = A3DString()
								a3dstr.name = os.path.basename(bpy.path.abspath(node.image.filepath)) if node.image and node.image.filepath else ""
			...
			if mats[x].use_nodes:
				for node in mats[x].node_tree.nodes:
					if node.type == 'TEX_IMAGE' and node.image is not None:
						...
						if node.image and node.image.filepath in linkedimgdata:
							#user already exists, retrieve ids
							imgid = linkedimgdata[node.image.filepath][0]
							linkedimg=True
						else:
							imgid = len(images)
							linkedimgdata[node.image.filepath if node.image and node.image.filepath else ""] = [imgid]
							linkedimg = False
...
class A3DImporter(bpy.types.Operator):
	...
	def execute(self, context):
		...
		file.close()
		print(".a3d import time: %.2f" % (time.perf_counter() - time1))
		return {'FINISHED'}
	def invoke (self, context, event):
...
class A3D2AmbientLight:
	...
	def render(self,objects):
		...
		lamp = bpy.data.lights.new(nme,"SUN")
...
class A3D2DirectionalLight:
	...
	def render(self,objects):
		...
		lamp = bpy.data.lights.new(nme,"AREA") 
		ob = bpy.data.objects.new(nme, lamp)
...
class A3D2OmniLight:
	...
	def render(self,objects):
		...
		lamp = bpy.data.lights.new(nme,"POINT") 
		ob = bpy.data.objects.new(nme, lamp)
...
class A3D2SpotLight:
	...
	def render(self,objects):
		...
		lamp = bpy.data.lights.new(nme,"SPOT") 
		ob = bpy.data.objects.new(nme, lamp)
		...
		for surf in self._surfaces:
			...
			if surf._materialId is not None:
				...
				if surf._materialId in materials:
					...
					#new material
					surf_mat = create_principled_material("Material")
					me.materials.append(surf_mat)

					# Assign material index to the polygons belonging to this surface
					if hasattr(surf, '_indexBegin') and hasattr(surf, '_numTriangles'):
						face_start = int(surf._indexBegin / 3) if surf._indexBegin is not None else 0
						face_count = surf._numTriangles if surf._numTriangles is not None else 0
						for fi in range(face_start, min(face_start + face_count, len(me.polygons))):
							me.polygons[fi].material_index = mat_index
					...
					if (mat._diffuseMapId is not None) and (mat._diffuseMapId != int("0xFFFFFFFF",16)):
						...
						diffuseimg = image
						add_texture_to_material(surf_mat, image, "diffuse")
					...
					if (mat._glossinessMapId is not None) and (mat._glossinessMapId != int("0xFFFFFFFF",16)):
						...
						DIR = os.path.dirname(self.Config.FilePath)
						image = load_image(img._url.rstrip('\0'), DIR)
						add_texture_to_material(surf_mat, image, "glossiness")
					...
					if (mat._lightMapId is not None) and (mat._lightMapId != int("0xFFFFFFFF",16)):
						...
						DIR = os.path.dirname(self.Config.FilePath)
						image = load_image(img._url.rstrip('\0'), DIR)
						add_texture_to_material(surf_mat, image, "light")
					...
					if (mat._normalMapId is not None) and (mat._normalMapId != int("0xFFFFFFFF",16)):
						...
						DIR = os.path.dirname(self.Config.FilePath)
						image = load_image(img._url.rstrip('\0'), DIR)
						add_texture_to_material(surf_mat, image, "normal")
					...
					if (mat._opacityMapId is not None) and (mat._opacityMapId != int("0xFFFFFFFF",16)):
						...
						DIR = os.path.dirname(self.Config.FilePath)
						image = load_image(img._url.rstrip('\0'), DIR)
						add_texture_to_material(surf_mat, image, "opacity")
					...
					if (mat._reflectionCubeMapId is not None) and (mat._reflectionCubeMapId != int("0xFFFFFFFF",16)):
						...
						DIR = os.path.dirname(self.Config.FilePath)
						image = load_image(img._url.rstrip('\0'), DIR)
						add_texture_to_material(surf_mat, image, "reflection")
					...
					if (mat._specularMapId is not None) and (mat._specularMapId != int("0xFFFFFFFF",16)):
						...
						DIR = os.path.dirname(self.Config.FilePath)
						image = load_image(img._url.rstrip('\0'), DIR)
						add_texture_to_material(surf_mat, image, "specular")

					mat_index += 1

		#set norms - use custom normals in Blender 4.x
		if len(norms) > 0:
			try:
				me.normals_split_custom_set_from_vertices(norms)
			except:
				pass  # skip if normals can't be set
				...
				if checkBMesh() == True:
					...
					for fc in range(len(uv_faces)):
						...
						if diffuseimg is not None:
							pass  # uv_layers image assignment removed in 4.x
...
class A3D2Skin:
	...
	def render(self,ibuffers,vbuffers,materials,maps,images,indexedJoints,joints,animationClips,animationTracks):
		...
		bone_names_in_table = {b[0] for b in boneTable1}
		sorted_bones = []
		added = set()
		remaining = list(boneTable1)
		max_iter = len(remaining) * len(remaining) + 1
		iteration = 0
		while remaining and iteration < max_iter:
			iteration += 1
			for bone in remaining[:]:
				bname, pname, transform = bone
				if pname is None or pname in added or pname not in bone_names_in_table:
					sorted_bones.append(bone)
					added.add(bname)
					remaining.remove(bone)
		sorted_bones.extend(remaining)  # fallback: add any unresolved
		boneTable1 = sorted_bones

		rig = self.createRig('Rig', (0,0,0), boneTable1)

		self.createAnimation(rig,animationClips,animationTracks)

		# Create vertex groups and skin the mesh to the rig
		if len(jnts) > 0 and len(joints) > 0:
			# Build joint ID -> bone name mapping
			joint_id_to_name = {}
			for jid in indexedJoints:
				joint_id_to_name[jid] = indexedJoints[jid]._name

			# Create vertex groups for each bone
			for jname in joint_id_to_name.values():
				if jname not in [vg.name for vg in ob.vertex_groups]:
					ob.vertex_groups.new(name=jname)

			# Count how many att=3 (joint) groups per vertex
			joint_attrs_per_vert = 0
			for v in self._vertexBuffers:
				vbuf = vbuffers[v]
				for att in vbuf._attributes:
					if att == 3:
						joint_attrs_per_vert += 1
				break

			# jnts has joint_attrs_per_vert entries per vertex
			# Each entry: (jointA_idx, jointA_weight, jointB_idx, jointB_weight)
			# Indices are GPU shader uniform indices (multiply by 3 per bone)
			# So divide by 3 to get the array index into self._joints
			num_joints = len(self._joints)
			num_verts = len(verts)
			assigned_count = 0
			for vi in range(num_verts):
				for ji in range(joint_attrs_per_vert):
					idx = vi * joint_attrs_per_vert + ji
					if idx >= len(jnts):
						break
					ai, aw, bi, bw = jnts[idx]
					ai_arr = int(round(ai)) // 3
					bi_arr = int(round(bi)) // 3
					if aw > 0.0 and ai_arr < num_joints:
						jid = self._joints[ai_arr]
						if jid in joint_id_to_name:
							grp = ob.vertex_groups.get(joint_id_to_name[jid])
							if grp:
								grp.add([vi], aw, 'ADD')
					if bw > 0.0 and bi_arr < num_joints:
						jid = self._joints[bi_arr]
						if jid in joint_id_to_name:
							grp = ob.vertex_groups.get(joint_id_to_name[jid])
							if grp:
								grp.add([vi], bw, 'ADD')
			...
			for v in ob.data.vertices:
				for vg in ob.vertex_groups:
					try:
						...
						pass
			print(f"Skinning: {assigned_count}/{num_verts} vertices assigned")
...
#		bpy.context.scene.update()
	...
	def createRig(self, name, origin, boneTable):
		...
		for (bname, pname, transform) in boneTable:
			...
			if pname and pname in amt.edit_bones:
				...
				# Parent bone not found, treat as root
				print(f"Warning: parent bone '{pname}' not found for '{bname}', treating as root")
				wmat = obmat
				bone.head = loc
			...
			bone.tail = rot.to_matrix() @ Vector(loc) + bone.head
			bpy.context.view_layer.update()
			...
			# amt.bones[bname].matrix = mat.to_3x3()  # read-only in 4.x
			bpy.ops.object.mode_set(mode='EDIT')
	...
	def createAnimation(self,arm_ob,animationClips,animationTracks):
		...
		# Build track lookup by bone name
		track_map = {}
		for track in animationTracks:
			if track._objectName in arm_ob.pose.bones:
				track_map[track._objectName] = track
			else:
				print(f"  Skipping track '{track._objectName}' - no matching bone")

		if not track_map:
			return

		# Get number of keyframes
		num_frames = max(len(t._keyframes) for t in track_map.values())

		# Sort bones in parent-first order
		sorted_bone_names = []
		for bone in arm_ob.data.bones:
			if bone.name in track_map:
				sorted_bone_names.append(bone.name)

		# Determine action name
		if animationClips and animationClips[0]._name:
			action_name = animationClips[0]._name
		else:
			action_name = "Action"

		arm_ob.animation_data_create()
		arm_ob.animation_data.action = bpy.data.actions.new(action_name)

		# A3D track matrices are LOCAL transforms (relative to parent)
		# Build world matrices by chaining: world = parent_world @ local
		# Then apply per-bone axis correction to convert from A3D to Blender bone axes

		# Compute per-bone axis correction from frame 0
		# At frame 0, the chained world matrix should equal bone.matrix_local
		# First, chain frame 0 locals to get frame0 world matrices
		frame0_worlds = {}
		for bone_name in sorted_bone_names:
			track = track_map[bone_name]
			if len(track._keyframes) == 0:
				continue
			local_mat = track._keyframes[0]._transform.getMatrix()
			pose_bone = arm_ob.pose.bones[bone_name]
			if pose_bone.parent and pose_bone.parent.name in frame0_worlds:
				frame0_worlds[bone_name] = frame0_worlds[pose_bone.parent.name] @ local_mat
			else:
				frame0_worlds[bone_name] = local_mat

		# correction: frame0_world.inv @ bone.matrix_local
		corrections = {}
		for bone_name in sorted_bone_names:
			if bone_name in frame0_worlds:
				rest_mat = arm_ob.data.bones[bone_name].matrix_local.copy()
				try:
					corrections[bone_name] = frame0_worlds[bone_name].inverted() @ rest_mat
				except:
					corrections[bone_name] = Matrix.Identity(4)

		# Apply animation frame by frame
		for frame in range(num_frames):
			bpy.context.scene.frame_set(frame)

			# Build world matrices for this frame
			frame_worlds = {}
			for bone_name in sorted_bone_names:
				track = track_map[bone_name]
				if frame >= len(track._keyframes):
					continue
				local_mat = track._keyframes[frame]._transform.getMatrix()
				pose_bone = arm_ob.pose.bones[bone_name]
				if pose_bone.parent and pose_bone.parent.name in frame_worlds:
					world_mat = frame_worlds[pose_bone.parent.name] @ local_mat
				else:
					world_mat = local_mat
				frame_worlds[bone_name] = world_mat

				# Apply bone axis correction
				if bone_name in corrections:
					corrected = world_mat @ corrections[bone_name]
				else:
					corrected = world_mat
...
class A3D2Sprite:
	...
	def render(self,materials,maps,images):
		...
		surf_mat = create_principled_material("SpriteMaterial")
		me.materials.append(surf_mat)
...
class A3d_submenu(bpy.types.Menu):
	bl_idname = "ALTERNATIVA3D_MT_submenu"
	...
	def draw(self, context):
		...
		layout.separator()
		layout.operator("a3dobj.a3d_ambientlight", text="AmbientLight", icon='OUTLINER_OB_LIGHT')
		layout.operator("a3dobj.a3d_directionallight", text="DirectionalLight", icon='OUTLINER_OB_LIGHT')
		layout.operator("a3dobj.a3d_omnilight", text="OmniLight", icon='OUTLINER_OB_LIGHT')
		layout.operator("a3dobj.a3d_spotlight", text="SpotLight", icon='OUTLINER_OB_LIGHT')
		layout.separator()
...
class AddSprite3D(bpy.types.Operator):
	...
	def execute(self, context):
		...
		me.materials.append(mat)
				# Texture setup handled via nodes (texture_slots removed in 4.x)
		mat.use_nodes = True
		return {'FINISHED'}
...
class AddAmbientLight(bpy.types.Operator):
	...
	def execute(self, context):
		lamp = bpy.data.lights.new("A3DAmbientLight","SUN")
...
class AddDirectionalLight(bpy.types.Operator):
	...
	def execute(self, context):
		lamp = bpy.data.lights.new("A3DDirectionalLight","AREA") 
...
class AddOmniLight(bpy.types.Operator):
	...
	def execute(self, context):
		lamp = bpy.data.lights.new("A3DOmniLight","POINT") 
...
class AddSpotLight(bpy.types.Operator):
	...
	def execute(self, context):
		lamp = bpy.data.lights.new("A3DSpotLight","SPOT") 
...
class alternativa3DPanel(bpy.types.Panel):
	...
	# bl_default_closed removed in 4.x
...
def menu_func(self, context):
	self.layout.menu("ALTERNATIVA3D_MT_submenu", icon="PLUGIN")
